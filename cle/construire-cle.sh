...
#
#   ./construire-cle.sh [--disque /dev/nvme0n1] [--iso CHEMIN]
#
...
mkdir -p "$travail/hub" "$travail/nocloud" "$travail/boot/grub" sortie
...
docker run --rm -u "$(id -u):$(id -g)" -v "$ISO:/iso/source.iso:ro" -v "$travail:/ajout:ro" -v "$PWD/sortie:/sortie" \
  hub-xorriso:1 xorriso -indev /iso/source.iso -outdev /sortie/hub-cle.iso \
    -map /ajout/hub /hub -map /ajout/nocloud /nocloud -map /ajout/boot/grub/grub.cfg /boot/grub/grub.cfg \
    -boot_image any replay 2>&1 | tail -3

printf '  ✓ sortie/hub-cle.iso (%s)\n' "$(du -h sortie/hub-cle.iso | cut -f1)"
