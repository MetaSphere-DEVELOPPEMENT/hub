...
class AutoriteLocaleTLS(AvecDossier):
    ...
    def test_racine_et_certificat_crees_cles_privees(self):
        a = self.autorite()
        self.assertTrue(a.preparer("192.168.1.50"))
        ...
        self.assertIn("IP Address:192.168.1.50", texte)
        ...
        self.assertEqual(a.contraintes(), {"IP:192.168.1.50/255.255.255.255", "DNS:hub.local", "DNS:salon.local"})
        self.assertRegex(a.empreinte(), r"^([0-9A-F]{2}:){31}[0-9A-F]{2}$")

    def test_duree_du_certificat_sous_les_limites_d_apple(self):
        a = self.autorite()
        a.preparer("192.168.1.50")
    ...
    def test_reemission_seulement_si_necessaire_racine_conservee(self):
        ...
        self.assertFalse(a.preparer("192.168.1.50"))
        self.assertEqual(a.crt.read_bytes(), serie)
        horloge.t += 370 * 86400
        self.assertTrue(a.a_renouveler())
        self.assertTrue(a.preparer("192.168.1.50"), "échéance proche : nouveau certificat")
    ...
    def test_ancienne_racine_trop_large_remplacee(self):
        ...
        a.fiche.write_text(json.dumps({"adresse": "192.168.1.50", "noms": T.noms_du_hub("salon"),
        ...
        self.assertFalse(a.preparer("192.168.1.50"), "remplacée une fois, pas à chaque démarrage")
    ...
    def test_refus_adresse_publique_horloge_fausse_openssl_absent(self):
        ...
        with self.assertRaises(T.ErreurTLS):
            self.autorite(horloge=lambda: 1_000_000_000).preparer("192.168.1.50")
        with self.assertRaises(T.ErreurTLS):
            self.autorite(openssl="").preparer("192.168.1.50")
...
class LectureDesContraintes(unittest.TestCase):
    ...
    OPENSSL3 = """        X509v3 extensions:
            ...
            X509v3 Name Constraints: critical
                Permitted:
                  IP:192.168.1.50/255.255.255.255
                  DNS:hub.local
                  DNS:salon.local
...
"""
    ...
    def test_lecture(self):
        self.assertEqual(T.lire_contraintes(self.OPENSSL3),
                         T.contraintes_attendues("192.168.1.50", ["hub.local", "salon.local"]))
