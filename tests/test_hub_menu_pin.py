...
class Verification(AvecReglages):
    def test_bon_code_ouvre_le_profil(self):
        self.ecrire({"id": "sam", "pin": hub_menu.hacher_pin("1234")}, {"id": "lea"})
    ...
    def test_le_code_d_un_autre_profil_n_ouvre_pas(self):
        self.ecrire({"id": "sam", "pin": hub_menu.hacher_pin("1234")}, {"id": "lea", "pin": hub_menu.hacher_pin("0000")})
        self.assertEqual(hub_menu.verifier_pin(self.c, ["lea"], "1234", maintenant=1000)["resultat"], "refus")
        self.assertEqual(hub_menu.verifier_pin(self.c, ["lea", "sam"], "1234", maintenant=1000)["profil"], "sam")
...
class Blocage(AvecReglages):
    ...
    def test_changer_de_profil_ne_redonne_pas_d_essais(self):
        self.ecrire({"id": "sam", "pin": hub_menu.hacher_pin("1234")}, {"id": "lea", "pin": hub_menu.hacher_pin("5678")})
        for ident in ("sam", "lea", "sam", "lea"):
            hub_menu.verifier_pin(self.c, [ident], "0000", maintenant=1000)
        self.assertEqual(hub_menu.verifier_pin(self.c, ["lea"], "0000", maintenant=1000)["attente"], 30)
